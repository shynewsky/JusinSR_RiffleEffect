...
CSlime::CSlime(const CSlime& Prototype)
	:CEnemy_SM{ Prototype }
{
}
...
void CSlime::Priority_Update(_float fTimeDelta)
{

}

int CSlime::Update(_float fTimeDelta)
{
	if (m_bDead)
		return OBJ_DEAD;
...
void CSlime::Late_Update(_float fTimeDelta)
{
	SetUp_OnTerrain(m_pTransformCom, 1.5f);
...
HRESULT CSlime::Ready_Components()
{
	//texture
	...
	_int start = m_bLeft ? 5 : 0;
	TextureDesc = { start, 0, 5 + start, 0.1f };
	if (FAILED(__super::Add_Component(LEVEL_SM, TEXT("Prototype_Component_Texture_Slime"),
...
HRESULT CSlime::Reset_RenderState()
{
...
CSlime* CSlime::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		MSG_BOX(TEXT("Failed To Create : CSlime"));
...
CGameObject* CSlime::Clone(void* pArg)
{
	...
	{
		MSG_BOX(TEXT("Failed To Clone : CSlime"));
		Safe_Release(pInstance);
	}
