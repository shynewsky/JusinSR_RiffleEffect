...
CKhaki::CKhaki(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CEnemy{ pGraphic_Device }
{
}

CKhaki::CKhaki(const CKhaki& Prototype)
	:CEnemy{ Prototype }
{
}

HRESULT CKhaki::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CKhaki::Initialize(void* pArg)
{
	...
	m_vOriginPos = Desc.vDir;
	m_iHP = m_iMaxHP = Desc.iMaxHP;
	m_fAttackRange = 90.f;
	m_CollisionMatrix = m_pTransformCom->Get_WorldMatrix();
	m_fFrictionRate = 0.85f;
	//srand((unsigned int)time(NULL));
	random_device rd;
	mt19937 gen(rd());
	uniform_real_distribution<float> randomX(1.f, 2.f);
	uniform_real_distribution<float> randomZ(1.f, 2.f);
	m_vRoll = _float3{ randomX(gen) * 0.1f , 0.f, randomZ(gen) * 0.1f };


	return S_OK;
}

void CKhaki::Priority_Update(_float fTimeDelta)
{
	if (m_iHP <= 0)
		return;
...
int CKhaki::Update(_float fTimeDelta)
{
	if (m_bDead)
	{
...
void CKhaki::Late_Update(_float fTimeDelta)
{
	...
	{
		m_bDeadSound = false;
		m_pGameInstance->PlayDefault(TEXT("Khaki_Hit.wav"), 27, 0.7f, false);
	...
	if (D3DXVec3Length(&(Get_Pos() - m_vOriginPos)) > 1.f)
	{
		m_vRoll =  m_vOriginPos - Get_Pos();
...
void CKhaki::IsPicked(_float3* pOutPos, _float* pDist)
{
	if (m_bDead)
		return;
...
HRESULT CKhaki::Ready_Components()
{
	//텍스쳐
	CTexture::TEXTUREDESC   TextureDesc;
	ZeroMemory(&TextureDesc, sizeof(CTexture::TEXTUREDESC));

 	if (FAILED(__super::Add_Component(LEVEL_MAZE, TEXT("Prototype_Component_Texture_Khaki_Move"),
	...
	if (FAILED(__super::Add_Component(LEVEL_MAZE, TEXT("Prototype_Component_Texture_Khaki_Die"),
	...
	if (FAILED(__super::Add_Component(LEVEL_MAZE, TEXT("Prototype_Component_Texture_Khaki_Roll"),
	...
	if (FAILED(__super::Add_Component(LEVEL_MAZE, TEXT("Prototype_Component_Texture_Khaki_Idle"),
...
HRESULT CKhaki::SetUp_RenderState()
{
...
HRESULT CKhaki::Reset_RenderState()
{
...
HRESULT CKhaki::AGGROPlayer()
{
	...
	else if (m_fDeltaPlayer <= m_fAttackRange * 1.5f)
	{
		m_eActLevel = ACT_AGGRO;
	}
	else
	{
		m_eActLevel = ACT_UNRECOGNIZE;
	}

	return S_OK;
}

void CKhaki::Act(_float _fTimeDelta)
{
	D3DXVec3Normalize(&m_vDeltaPlayer, &m_vDeltaPlayer);

	switch (m_eActLevel)
	{
	case CEnemy::ACT_UNRECOGNIZE:
...
void CKhaki::CameraAround()
{
	m_pGraphic_Device->GetTransform(D3DTS_VIEW, &m_mCamreraWorld);

	D3DXMatrixInverse(&m_mCamreraWorld, nullptr, &m_mCamreraWorld);
	
	_float3 Looking = CFSM::OBJSTATE_ROLL == m_pFSM->Get_CurState()? m_vInertia : m_vDeltaPlayer ;

	//cos22.5 = 0.923880
	//cos67.5 = 0.382683
	_float zDot = D3DXVec3Dot(&Looking, &(*(_float3*)(&m_mCamreraWorld._31)));
	...
	_float xDot = D3DXVec3Dot(&Looking, &(*(_float3*)(&m_mCamreraWorld._11)));
...
CKhaki* CKhaki::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CKhaki* pInstance = new CKhaki(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("failed to Create Prototype Khaki"));
...
CGameObject* CKhaki::Clone(void* pArg)
{
	CKhaki* pInstance = new CKhaki(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("failed to Clone Khaki"));
		Safe_Release(pInstance);
	}
