...
CUINumber::CUINumber(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CImage{ pGraphic_Device }
{
}

CUINumber::CUINumber(const CUINumber & Prototype)
	: CImage{ Prototype }
{
}
...
void CUINumber::Priority_Update(_float fTimeDelta)
{
}

int CUINumber::Update(_float fTimeDelta)
{
	...
	//cout << m_iSkillCoolTime << endl;

	return OBJ_NOEVENT;
}

void CUINumber::Late_Update(_float fTimeDelta)
{
...
HRESULT CUINumber::Render()
{
	...
	if (FAILED(m_pTextureCom->Bind_Texture((int)m_fSkillCoolTime+1)))
		return E_FAIL;
...
void CUINumber::Set_Variables(void* pArg)
{
...
void CUINumber::Set_OrthoLH()
{
	...
	m_pTransformCom->Set_State(CTransform::STATE_POSITION,
		_float3(m_tImageInfo.fX - g_iWinSizeX * 0.5f, -m_tImageInfo.fY + g_iWinSizeY * 0.5f, 0.004f));

	D3DXMatrixOrthoLH(&m_ProjMatrix, g_iWinSizeX, g_iWinSizeY, 0.0f, 1.f);
}

CUINumber * CUINumber::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CUINumber*		pInstance = new CUINumber(pGraphic_Device);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created : CUINumber"));
