...
CRed::CRed(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CEnemy{ pGraphic_Device }
{
}

CRed::CRed(const CRed& Prototype)
	:CEnemy{ Prototype }
{
}

HRESULT CRed::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CRed::Initialize(void* pArg)
{
...
void CRed::Priority_Update(_float fTimeDelta)
{
	if (m_iHP <= 0)
		return;
...
int CRed::Update(_float fTimeDelta)
{
	if (m_bDead)
	{
...
void CRed::Late_Update(_float fTimeDelta)
{
...
void CRed::IsPicked(_float3* pOutPos, _float* pDist)
{
	if (m_bDead)
		return;
...
HRESULT CRed::Ready_Components()
{
	//텍스쳐
	CTexture::TEXTUREDESC   TextureDesc;
	ZeroMemory(&TextureDesc, sizeof(CTexture::TEXTUREDESC));

	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Red_Move"),
	...
	if (FAILED(__super::Add_Component(LEVEL_GAMEPLAY, TEXT("Prototype_Component_Texture_Red_Roll"),
...
HRESULT CRed::SetUp_RenderState()
{
...
HRESULT CRed::Reset_RenderState()
{
...
void CRed::Act(_float _fTimeDelta) 
{
	...
	case CEnemy::ACT_ATTACK:
		if (m_fAttackCooltime < 0.f)
		{
			...
			CBullet_Arc::BULLETARC_DESC Desc = { m_pTransformCom->Get_State(CTransform::STATE_POSITION),
			m_pTransformPlayer->Get_State(CTransform::STATE_POSITION), m_pTerrainVIBuffer, m_pTerrainTransform };
			if (FAILED(m_pGameInstance->Add_CloneObject_ToLayer(m_eCurLevel, TEXT("Layer_Bullet"), TEXT("Prototype_GameObject_Bullet_Arc"), &Desc)))
				return;
...
CRed* CRed::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CRed* pInstance = new CRed(pGraphic_Device);
