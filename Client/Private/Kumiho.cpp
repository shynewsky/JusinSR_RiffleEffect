...
CKumiho::CKumiho(LPDIRECT3DDEVICE9 pGraphic_Device)
	:CEnemy_SM{ pGraphic_Device }
{
}

CKumiho::CKumiho(const CKumiho& Prototype)
	:CEnemy_SM{ Prototype }
{
}

HRESULT CKumiho::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CKumiho::Initialize(void* pArg)
{
...
void CKumiho::Priority_Update(_float fTimeDelta)
{

}

int CKumiho::Update(_float fTimeDelta)
{
	if (m_bDead)
		return OBJ_DEAD;
...
void CKumiho::Late_Update(_float fTimeDelta)
{
...
HRESULT CKumiho::Ready_Components()
{
	//texture
	...
	_int start = m_bLeft ? 0 : 6;
	TextureDesc = { start, 0, 6 + start, 0.1f };
	if (FAILED(__super::Add_Component(LEVEL_SM, TEXT("Prototype_Component_Texture_Kumiho_Move"),
...
HRESULT CKumiho::SetUp_RenderState()
{
...
HRESULT CKumiho::Reset_RenderState()
{
...
CKumiho* CKumiho::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CKumiho* pInstance = new CKumiho(pGraphic_Device);
	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Create : CKumiho"));
...
CGameObject* CKumiho::Clone(void* pArg)
{
	CKumiho* pInstance = new CKumiho(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Clone : CKumiho"));
		Safe_Release(pInstance);
	}
